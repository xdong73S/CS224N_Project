...
import match_lstm_modules as match_layers
import torch
...
class MatchLSTM(nn.Module):
    def __init__(self, word_vectors, char_vectors, hidden_size, 
        ...
        self.emb = match_layers.Embedding(hidden_size=hidden_size,
                                    word_drop_prob=drop_prob,
                                    char_drop_prob=char_drop_prob,
                                    use_char_emb=use_char_emb)

        self.enc = match_layers.RNNEncoder(input_size=hidden_size,
                                     hidden_size=hidden_size,
                                     num_layers=1,
                                     drop_prob=drop_prob)

        encode_out_size = hidden_size # bidirectional
        self.match_rnn = match_layers.MatchRNN("LSTM", encode_out_size, encode_out_size, hidden_size, bidirectional=True, gated_attention=True, dropout_p=drop_prob, enable_layer_norm=True)

        match_rnn_out_size = hidden_size * 2

        self.pointer_net = match_layers.BoundaryPointer("LSTM", match_rnn_out_size, hidden_size, bidirectional=False, dropout_p=drop_prob, enable_layer_norm=True)
    ...
    def forward(self, cw_idxs, cc_idxs, qw_idxs, qc_idxs):
        ...
        c_enc.transpose_(0,1)
        q_enc.transpose_(0,1)

        real_rtn_hidden, last_hidden, rtn_para = self.match_rnn.forward(c_enc, c_mask, q_enc, q_mask)

        ans_range_prop = self.pointer_net.forward(real_rtn_hidden, c_mask)
        ans_range_prop = ans_range_prop.transpose(0, 1)
    
        _, ans_range = torch.max(ans_range_prop, dim=2)

        return ans_range_prop, ans_range
